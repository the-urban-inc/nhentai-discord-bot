...
# About
nhentai is an open source nhentai Discord bot powered by [TypeScript](https://www.typescriptlang.org/) with [discord.js](https://discord.js.org/#/).

##### Regarding `n!` commands : it's gone. Implementing `n!` commands require accessing each and every message's content (to respond when you call a command). For larger bots, this requires whitelisting from Discord's side - and they won't do that just for old-style commands to work, instead pushing people to use slash commands instead.
