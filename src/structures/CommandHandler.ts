...
export class CommandHandler extends ApplicationCommandManager {
    ...
    private async handleInteraction(interaction: Interaction) {
        if (!('isCommand' in interaction) || (!interaction.isCommand() && !interaction.isAnySelectMenu() && !interaction.isAutocomplete())) return;
        if (!(interaction.channel instanceof TextChannel) || interaction.channel instanceof ThreadChannel) return;
        if (!([InteractionType.ApplicationCommand, InteractionType.ApplicationCommandAutocomplete].includes(interaction.type as InteractionType))) return;
        ...
        try {
            ...
            if (interaction.isAutocomplete()) {
                ...
                this.client.logger.info(`[COMMAND][autocomplete] ${interaction.user.tag} (ID: ${interaction.user.id}) => ${(interaction as any).commandName} completed in ${acElapsed}ms`);
                return;
            }
            let server = await Server.findOne({ serverID: interaction.guild.id }).exec();
            if (!server) {
                server = await new Server({ serverID: interaction.guild.id, settings: { private: false } }).save();
            }
            ...
            if ((interaction as any).isCommand && (interaction as any).isCommand()) {
                ...
                const privateOpt = (cmdInteraction.options.get('private')?.value as boolean) ?? privateCommand;
            ...
            if (!interaction.channel.nsfw && nsfw) {
                throw new UserError('NSFW_COMMAND_IN_SFW_CHANNEL', (interaction as any).commandName);
            }

            if (permissions) {
                const userPerms = interaction.channel.permissionsFor(interaction.user);
                if (!userPerms || !userPerms.has(permissions)) {
                    const missing = permissions.filter((p: any) => !userPerms.has(p)).map((p: any) => this.client.util.resolvePerm(p));
            ...
            try {
                ...
                this.client.logger.info(`[COMMAND] ${interaction.user.tag} (ID: ${interaction.user.id}) => ${name} completed in ${execElapsed}ms`);
                startCooldown();
            } catch (err) {
                const failedElapsed = Math.round(performance.now() - execStart);
                this.client.logger.info(`[COMMAND] ${interaction.user.tag} (ID: ${interaction.user.id}) => ${(interaction as any).commandName} failed after ${failedElapsed}ms`);
                throw err;
            }
        } catch (err: any) {
            if (axios.isAxiosError(err) || err.code === 10062 || err.code === 40060) return this.client.logger.error(`${(interaction as any)?.commandName} => ${err.message}`);
            else this.client.logger.error(err instanceof UserError ? err.code : err);
            if ((interaction as any).isAutocomplete && (interaction as any).isAutocomplete()) return;
            const type = (interaction as any).deferred || (interaction as any).replied ? 'editReply' : 'reply';
            if (err instanceof UserError) {
                if (['NO_RESULT', 'INVALID_PAGE_INDEX', 'UNKNOWN_TAG'].includes(err.code)) startCooldown();
                return (interaction as any)[type]({ content: err.message, embeds: [], components: [] });
            }
            return (interaction as any)[type]({ content: `An unexpected error occurred while executing the command \`${(interaction as any).commandName}\`: \`\`\`${err.message}\`\`\``, embeds: [], components: [] });
        }
    }
    ...
    cloneCommandData(c: Command, rep: string) {
        ...
        delete C.data.clone;
        C.data = JSON.parse(JSON.stringify(C.data).replace(new RegExp(clone.keyword, 'g'), rep)) as Command['data'];
        return C;
    }
    ...
    async loadCommands() {
        try {
            ...
            const commands = await Promise.all(
                commandFiles.map(async file => {
                    ...
                    if ((c as Command).data.clone) {
                        const cloned = c.data.clone.clones.map(clone => this.cloneCommandData(c, clone));
                        return cloned;
                    }
                    return [c];
                })
            ...
            updatedCommands = await this.client.application?.commands.set(allCommands.map(c => c.data));
            allCommands.forEach(cc => {
                const cmd = updatedCommands.find(dc => dc.name === cc.data.name);
                if (!cmd) return;
                this.client.commands.set(cmd.name, cc);
            });
            this.client.logger.info(`[COMMANDS] Loaded ${this.client.commands.size} commands successfully.`);
        } catch (err) {
