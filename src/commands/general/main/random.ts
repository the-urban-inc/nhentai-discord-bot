...
export default class extends Command {
	...
	async exec(message: Message, { auto }: { auto: boolean }) {
        try {
            ...
            const displayDoujin = this.client.embeds.richDisplay({ auto: auto }).setGID(id).setInfoPage(info);
            ...
            const displayComments = this.client.embeds.richDisplay({ love: false }).useCustomFooters();
