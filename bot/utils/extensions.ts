...
export function calculateLevel(exp: number) {
    ...
    const findPos = (a: Array<number>, x: number) => { for (let i = 0; i < a.length; i++) if (a[i] > x) return i; };
 ...
 * @param {string[]}} tags 
 */
export function gshorten(tags: Array<string>) {
    let res = '';
...
export function random(array: Array<any>) {
    return array[Math.floor(Math.random() * array.length)];
}

export function shorten(text: string, split = ' ', maxLen = 2000) {
